...
class LoginTest(unittest.TestCase):
    ...
    def testlogin2(self):
            # form = {"TestUserName":"111", "TestUserPWD":"111"}
            form = {"xajax": "xCheckUserLogin", "xajaxr": "1574386024465",
                    "xajaxargs[]": "<xjxquery><q>TestUserName=hjwg&TestUserPWD=geee&Language=ZH_CN_UTF-8&HttpRefer=</q></xjxquery>"}
            r = requests.post(self.url, data=form)
     #       print(r.status_code)
        #    print(r.text)
            self.assertIn("您的用户名或密码不正确", r.text)
...
def suite():
    ...
    # alltest = unittest.TestSuite(loginTestCase, logoutTestCase)
    # return alltest
